...
export type WebResearchInput = {
    ...
    mode: "quick" | "deep";
    market: string;
    // NOTE: keep exactOptionalPropertyTypes happy: optional WITHOUT explicit "| undefined" is cleaner.
    language?: string;
    topic?: "seasonal" | "product" | "supplier" | "general";
};

function needsExpansion(json: any, input: WebResearchInput, evidenceCount: number) {
    if (evidenceCount <= 0) return false;
    if (input.mode !== "deep") return false;
...
function deriveClusterBundlesFromRows(rows: any[], maxClusters: number) {
    ...
    return clusters.map(({ cluster, items }) => {
        ...
        const actions =
            recommendedActions.length > 0
                ? recommendedActions
                : [{ title: `Create Etsy/Shopify listing SEO set for: ${cluster}`, priority: "P1" as const }];
        ...
        return {
            cluster,
            topKeywords,
            recommendedActions: actions.map((a: any) => ({
...
function backfillClusterBundlesFromRows(rows: any[], maxClusters: number) {
    ...
    return Array.from(by.entries())
        ...
        .map(([cluster, items]) => ({
            ...
            recommendedActions: [{ title: `Create SEO set for: ${cluster}`, priority: "P1" as const }],
            topEvidence: items
                .flatMap((x) => x.topEvidence ?? [])
                .filter((e: any) => e?.url && e?.title)
                .slice(0, 2)
                .map((e: any) => ({ url: e.url, title: e.title })),
        }));
}
...
export async function runResearchPipeline(input: WebResearchInput) {
    ...
    const plan =
        input.mode === "deep"
            ? { maxResultsPerQuery: 10, searchDepth: "advanced" as const }
            : { maxResultsPerQuery: 8, searchDepth: "basic" as const };
        ...
        for (const r of allResults[i] ?? []) {
            try {
                ...
                evidence.push({
                    ...
                    publishedAt: (r as any).published_date ?? null,
                    capturedAt: nowIso,
                    domain: u.hostname,
                    query: q,
                });
    ...
    const ensureNonEmpty = (v: unknown, fallback: string) =>
        typeof v === "string" && v.trim().length > 0 ? v : fallback;

    const hardenResultBundle = (json: any) => {
        ...
        json.resultBundle.title = ensureNonEmpty(
            json.resultBundle.title,
            `Web Research: ${input.prompt}`
        );
        json.resultBundle.summary = ensureNonEmpty(
            json.resultBundle.summary,
            "Summary unavailable due to insufficient structured output."
        );

        if (!Array.isArray(json.resultBundle.nextSteps)) json.resultBundle.nextSteps = [];
        if (!Array.isArray(json.resultBundle.sources)) json.resultBundle.sources = [];
    };
